...
Configuration:
  Uses .env file if present, or set environment variables:
    ...
    CA_ARN, ACM_CERT_ARN (printed by setup.sh on completion)
EOF
  exit 1
}
...
echo "=== Removing Private CA ==="
aws acm-pca update-certificate-authority --certificate-authority-arn "$CA_ARN" --status DISABLED --region "$AWS_REGION" 2>/dev/null || true
aws acm-pca delete-certificate-authority --certificate-authority-arn "$CA_ARN" --region "$AWS_REGION" 2>/dev/null || true
